				...
				<div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 w-full mx-auto mt-2 mb-10 animate-pulse">
					{Array.from({ length: 6 }).map((_, idx) => (
						<div key={idx} className="bg-neutral-50 rounded-lg shadow flex flex-col h-full">
							...
							{/* Image Skeleton */}
							<div className="w-full h-20 sm:h-32 rounded-md bg-gray-200 mb-3 relative"></div>
							
							{/* Title Skeleton */}
							<div className="h-5 bg-gray-200 rounded mx-4 mb-2"></div>
							<div className="h-4 bg-gray-200 rounded mx-4 mb-3 w-3/4"></div>
							
							{/* Divider */}
							<div className="h-px bg-gray-200 mx-4 mb-3"></div>
							...
							<div className="mt-auto flex flex-col md:flex-row justify-end gap-2 px-4 pb-4">
								<div className="h-8 bg-gray-200 rounded w-full sm:w-auto"></div>
								<div className="h-8 bg-gray-200 rounded w-full sm:w-auto"></div>
							</div>
			...
			<div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-[1500px]">
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 w-full mx-auto mt-2 mb-10 bg-neutral-50 p-4 sm:p-6 rounded-lg shadow-sm">
					...
					<div key={idx} className="bg-white rounded-lg shadow hover:shadow-md transition flex flex-col h-full">
						...
						{/* Action Buttons */}
						<div className="mt-auto flex flex-col md:flex-row justify-end gap-2 px-4 pb-4">
							<button onClick={() => {setPdfFile(course.subject_url); setIsPreviewOpen(true);}} className="flex items-center justify-center gap-2 border border-gray-300 rounded-md px-4 py-2 text-gray-700 hover:bg-gray-100 transition text-sm w-full sm:w-auto">
								Preview
							</button>

							<button onClick={() => downloadPDF(course.subject_url, course.name)} className="flex items-center justify-center gap-2 bg-primary-500 text-white rounded-md px-4 py-2 hover:bg-blue-700 transition text-sm w-full sm:w-auto">
